...
def trial(model_version, model, batch_size, criterion, scheduler, opt):
    # 创建dataloader
    train_data_loader_list, valid_data_loader_list = get_data_loader(mode='train', batch_size=batch_size)   
    
    test_data_loader = get_data_loader(mode='test', batch_size=batch_size)
...
#         model = ADMET()
        train_data_loader, valid_data_loader = train_data_loader_list[i], valid_data_loader_list[i]
    ...
    #             label_true_batch = label_true_batch.to(device)
            ...
            test_pred = test(model, test_data_loader)
            ...
            if score > current_best_metric:
                ...
                pdl.save(model.state_dict(), "weight/" + model_version + "_" + str(i) + ".pkl")
            print("=========================================================")
            print("KFold, Epoch", i, epoch)
            ...
            if epoch > current_best_epoch + max_bearable_epoch:
                break
        best_test_pred_list.append(best_test_pred)

    return train_metric_list, valid_metric_list, best_test_pred_list
...
metric_train_list, metric_valid_list, best_test_pred_list = trial(model_version='3', model=model, batch_size=batch_size, criterion=criterion, scheduler=scheduler, opt=opt)

best_test_pred_list = np.array(best_test_pred_list)
np.save('./result/GEM/3.npy',best_test_pred_list)