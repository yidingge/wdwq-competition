...
def get_data_loader(mode, batch_size):
    if mode == 'train':
        ...
        for t,v in kf.split(data_list):
#             print(t)
            train_list = [data_list[k] for k in t]
            valid_list = [data_list[k] for k in v]
            train_dataset = InMemoryDataset(train_list)
            valid_dataset = InMemoryDataset(valid_list)
        ...
        return train_data_loader_list, valid_data_loader_list
